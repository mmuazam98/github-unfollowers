...
const Search = forwardRef(({ handleSearch }: { handleSearch: () => void }, ref) => {
 ...
 return (
  ...
  >
   ...
   <IconButton onClick={handleSearch} data-testid="search-button">
    <SearchIcon />
   </IconButton>
  </Box>
 );
...
const CustomLabel = ({ label, count }: { label: string; count: number }) => {
 return (
  <Typography sx={{ fontWeight: "bold" }}>
   {label}{" "}
   <span style={{ fontWeight: "normal" }} data-testid={label}>
    ({count})
   </span>
  </Typography>
 );
   ...
   <Box className="tabs">
    <Box sx={{ borderBottom: 1, borderColor: "divider" }}>
     <Tabs value={value} onChange={handleChange} aria-label="basic tabs example">
      <Tab label={<CustomLabel label="followers" count={info?.followers?.length || 0} />} {...a11yProps(0)} />
      <Tab label={<CustomLabel label="following" count={info?.following?.length || 0} />} {...a11yProps(1)} />
      <Tab label={<CustomLabel label="unfollowers" count={info?.unfollowers?.length || 0} />} {...a11yProps(2)} />
      <Tab label={<CustomLabel label="not mutual" count={info?.notMutual?.length || 0} />} {...a11yProps(3)} />
     </Tabs>
    </Box>
    <TabPanel value={value} index={0}>
     {!info?.followers?.length ? <LoadingContent /> : info?.followers.map((user: any, idx: number) => <User user={user} key={user.node_id} />)}
    </TabPanel>
